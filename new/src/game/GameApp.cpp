...
namespace autochess::game
{
    namespace
    {
        ...
        constexpr std::uint64_t SettlementHoldFrames = 180U;
    }
    ...
    int GameApp::run()
    {
        ...
        if (!loadConfiguration() || !loadFonts())
        {
            return 1;
        }
    ...
    // 此函数优先加载宋体、New York 和微软雅黑粗体，并提供稳定回退。
    bool GameApp::loadFonts()
    {
        const std::array<std::filesystem::path, 3> chineseCandidates = {
            std::filesystem::path("C:/Windows/Fonts/simsun.ttc"),
            std::filesystem::path("C:/Windows/Fonts/STSONG.TTF"),
            dataDirectory_ / "fonts" / "NotoSansSC-VF.ttf"};
        const std::array<std::filesystem::path, 4> englishCandidates = {
            dataDirectory_ / "fonts" / "NewYork.ttf",
            dataDirectory_ / "fonts" / "NewYork-Regular.ttf",
            std::filesystem::path("C:/Windows/Fonts/times.ttf"),
            dataDirectory_ / "fonts" / "NotoSansSC-VF.ttf"};
        const std::array<std::filesystem::path, 3> guardCandidates = {
            std::filesystem::path("C:/Windows/Fonts/msyhbd.ttc"),
            std::filesystem::path("C:/Windows/Fonts/msyh.ttc"),
            dataDirectory_ / "fonts" / "NotoSansSC-VF.ttf"};

        bool chineseLoaded = false;
        for (const std::filesystem::path& path : chineseCandidates)
        {
            if (std::filesystem::is_regular_file(path)
                && chineseFont_.loadFromFile(path.string()))
            {
                chineseLoaded = true;
                break;
            }
        ...
        for (const std::filesystem::path& path : englishCandidates)
        {
            if (std::filesystem::is_regular_file(path)
                && englishFont_.loadFromFile(path.string()))
            {
                englishLoaded = true;
                break;
            }
        ...
        for (const std::filesystem::path& path : guardCandidates)
        {
            if (std::filesystem::is_regular_file(path)
                && guardFont_.loadFromFile(path.string()))
            {
                guardLoaded = true;
                break;
            }
        ...
        if (!chineseLoaded)
        {
            std::cerr << "[FONT] 无法加载宋体中文字体\n";
        }
        if (!englishLoaded)
        {
            std::cerr << "[FONT] 无法加载 New York/Times 英文字体\n";
        }
        if (!guardLoaded)
        {
            std::cerr << "[FONT] 无法加载微软雅黑守卫值字体\n";
        }
        return chineseLoaded && englishLoaded && guardLoaded;
    }
    ...
    {
        clearMatchState();
        screen_ = std::make_unique<MainMenuScreen>(chineseFont_);
    }

    // 此函数用统一字体构造没有残留交互状态的帮助页面。
    void GameApp::showHelp()
    {
        screen_ = std::make_unique<HelpScreen>(chineseFont_);
    }
    ...
    void GameApp::startNewGame()
    {
        ...
        computerController_ = std::make_unique<core::AiController>(
            config_,
            core::MapSide::B);
        screen_ = std::make_unique<MatchScreen>(
            chineseFont_, englishFont_, guardFont_, config_, dataDirectory_);
    ...
    void GameApp::fixedUpdate()
    {
            ...
            && settlementHoldFrames_ > 0)
        {
            ...
            if (auto* matchScreen = dynamic_cast<MatchScreen*>(screen_.get()))
            {
                matchScreen->setSettlementCountdown(settlementHoldFrames_);
        ...
        {
            settlementHoldFrames_ = SettlementHoldFrames;
            if (auto* matchScreen = dynamic_cast<MatchScreen*>(screen_.get()))
            {
                matchScreen->setSettlementCountdown(settlementHoldFrames_);
            }
