...
namespace autochess::game
{
        ...
        spine::SkeletonData& skeletonData)
    {
        ...
        int dieRank = 100;
        std::string legacyAttackBegin;
        std::string legacyAttackLoop;
        std::string legacyAttackEnd;
             ...
             ++index)
        {
            ...
            {
                legacyAttackBegin = originalName;
            }
            else if (normalizedName == "skill_1_loop")
            {
                legacyAttackLoop = originalName;
            }
            else if (normalizedName == "skill_1_end")
            {
                legacyAttackEnd = originalName;
            }
        }

        // 少数老师素材使用历史遗留的 Skill_1_Start/Loop/End 片段名表示
        // 普通攻击；这里仅兼容素材命名，不关联任何玩法技能系统。
        if (manifest.attack.empty() && !legacyAttackLoop.empty())
        {
            manifest.attack = legacyAttackLoop;
            if (!legacyAttackBegin.empty() && !legacyAttackEnd.empty())
            {
                manifest.attackBegin = legacyAttackBegin;
                manifest.attackEnd = legacyAttackEnd;
            }
        }
