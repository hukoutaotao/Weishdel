...
namespace autochess::core
{
    namespace
    {
            ...
            const ConfigBundle& config)
        {
            ...
            if (skill->effectType == SkillEffectType::Buff
                && (skill->buffStat == BuffStat::MoveSpeed
                    || skill->buffStat == BuffStat::AttackSpeed))
            {
                return 300;
            }
            if (skill->effectType == SkillEffectType::Buff)
    ...
    std::optional<GameCommand> RouteStrategy::chooseCombatCommand(
        const MapSide actor,
        const ReadOnlyGameView& view,
        const ConfigBundle& config) const
    {
        // 此分支只允许路线型策略在战斗阶段寻找主动技能。
