...
namespace autochess::core
{
    ...
    const SkillDefinition* AiDecisionSupport::findSkill(
        const ConfigBundle& config,
        const std::string& skillId) noexcept
    {
        // 此循环只读取正式技能配置并返回匹配技能的稳定地址。
        const auto iterator = std::find_if(
            config.skills.begin(),
            config.skills.end(),
            [&skillId](const SkillDefinition& definition)
            {
                return definition.id == skillId;
            });
        return iterator == config.skills.end() ? nullptr : &*iterator;
    }
    ...
    bool AiDecisionSupport::skillCanRelease(
        const BattleUnit& caster,
        const ReadOnlyGameView& view,
        const ConfigBundle& config) noexcept
    {
        // 此分支先检查战斗单位自身的技力、存活和技能状态。
        if (caster.side != view.viewer
            || caster.state != BattleUnitState::Alive
            || caster.activeSkill.active
            || caster.maxMana <= 0.0
            || caster.currentMana < caster.maxMana)
        {
            return false;
        }
        const SkillDefinition* skill = findSkill(config, caster.skillId);
        if (skill == nullptr)
        {
            return false;
        }
        ...
        // 此循环检查技能是否存在至少一个合法的敌人或受伤友军目标。
        for (const BattleUnit& candidate : view.battleUnits)
        {
            if (candidate.state != BattleUnitState::Alive
                || candidate.id == InvalidBattleUnitId)
            {
                continue;
            }
